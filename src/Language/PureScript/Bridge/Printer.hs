...
{-|
 - Configuration for the bridge. Currently supported options:
 ...
 -      'moduleToTextNoOptics': Standard generator without optics.
-}
data Config = Config {
  _cModuleToText :: Module 'PureScript -> Text
}
...
moduleToTextNoOptics :: Module 'PureScript -> Text
...
printModule :: FilePath -> PSModule -> BridgeM ()
...
moduleToText' :: [ImportLine] -> (SumType 'PureScript -> Text) -> Module 'PureScript -> Text
moduleToText' otherImports sumTypeToText' m = T.unlines $
...
sumTypeToText st =
  sumTypeToTextNoOptics st
    <> "\n"
    <> sumTypeToPrismsAndLenses st
    <> separator

sumTypeToTextNoOptics :: SumType 'PureScript -> Text
sumTypeToTextNoOptics st =
